...
public class ParticipationUserController {
	...
	public ModelAndView create(@RequestParam int q) {
		...
		List<Raffle> rafflesBaned = raffleService.raffleManagerIsDebtor();
		if (today.before(raffle.getDeadline()) && today.after(raffle.getPublicationTime()) && !rafflesBaned.contains(raffle)) {
	...
	public ModelAndView saveCreate(@Valid Participation participation, BindingResult binding) {
		...
		if (!today.after(raffleService.findOne(participationId).getPublicationTime()) && !today.before(raffleService.findOne(participationId).getDeadline()) && !rafflesBaned.contains(raffleService.findOne(participationId))) {
			result = new ModelAndView("redirect:/welcome/index.do");
		}
